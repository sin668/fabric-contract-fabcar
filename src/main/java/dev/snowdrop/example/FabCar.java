...
public final class FabCar implements ContractInterface {
    ...
    private final static String CAR_COMPOSITE_KEY = "myfabcarkey";
    ...
    @Transaction()
    public String queryByOwner(final Context ctx, final String owner) {
        List<CarQueryResult> queryResults = new ArrayList<CarQueryResult>();
        ChaincodeStub stub = ctx.getStub();
        QueryResultsIterator<KeyValue> results = stub.getStateByPartialCompositeKey(CAR_COMPOSITE_KEY, owner); 
        ...
        if (results != null && results.iterator().hasNext()) {
            ...
            String errorMessage = String.format("Owner %s have not car exist", owner);
            System.out.println(errorMessage);
            throw new ChaincodeException(errorMessage, FabCarErrors.CAR_NOT_FOUND.toString());
        }
    ...
    @Transaction()
    public Car createCar2(final Context ctx, final String key, final String make, final String model,
            final String color, final String owner) {
        ChaincodeStub stub = ctx.getStub();
        ...
        CompositeKey compositeKey = stub.createCompositeKey(CAR_COMPOSITE_KEY, owner, key);
        Car car = new Car(make, model, color, owner);
        carState = genson.serialize(car);
        stub.putStringState(compositeKey.toString(), carState);
    ...
    @Transaction()
    public String history(final Context ctx, final String type, final String key) {
        ChaincodeStub stub = ctx.getStub();

        List<CarHistoryResult> queryResults = new ArrayList<CarHistoryResult>();

        QueryResultsIterator<KeyModification> results = stub.getHistoryForKey(key);

        for (KeyModification result: results) {
            Car car = genson.deserialize(result.getStringValue(), Car.class);
            queryResults.add(new CarHistoryResult(result.getTxId(), result.getTimestamp().toString(), 
                    result.isDeleted(), car));
        }
    ...
    public static void main(String[] args) {
        byte[] ccb;
		try {
			ccb = Files.readAllBytes(Paths.get("/projects/fabric-contract-sample/src/main/resources/peer/cert.crt"));
            System.out.println(new String(Base64.getEncoder().encode(ccb), "UTF-8"));
		} catch (IOException e) {
